...
compute_descriptive_stats <- function(returns_list) {
  ...
  for (tkr in names(returns_list)) {
    ...
    out <- rbind(out, data.frame(
      ...
      jb_pvalue       = round(as.numeric(jb$p.value), 8),
...
run_stationarity_tests <- function(returns_list, alpha = 0.05) {
  ...
  for (tkr in names(returns_list)) {
    ...
    out <- rbind(out, data.frame(
      ...
      adf_pvalue      = round(as.numeric(adf$p.value),  8),
      adf_stationary  = adf_stationary,
      kpss_stat       = as.numeric(kpss$statistic),
      kpss_pvalue     = round(as.numeric(kpss$p.value), 8),
      kpss_stationary = kpss_stationary,
      pp_stat         = as.numeric(pp$statistic),
      pp_pvalue       = round(as.numeric(pp$p.value),   8),
  ...
  for (tkr in names(returns_list)) {
    ...
    for (L in lags) {
      ...
      out <- rbind(out, data.frame(
        ...
        p_value     = round(as.numeric(lb_r$p.value), 8),
      ...
      out <- rbind(out, data.frame(
        ...
        p_value     = round(as.numeric(lb_r2$p.value), 8),
    ...
    out <- rbind(out, data.frame(
      ...
      p_value     = round(as.numeric(arch$p.value), 8),
